import 'package:flutter/material.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';

Widget getWeatherIcon({required String? weatherDescription, required Color color, required double size})
{
  switch(weatherDescription)
  {
    case "Clear":
    {
      return Icon(FontAwesomeIcons.sun, color: Color(0xFFFFB74D), size: size);
    }
    case "Clouds":
    {
      return Icon(FontAwesomeIcons.cloud, color: Color(0xFFBDBDBD), size: size);
    }
    case "Rain":
      {
        return Icon(FontAwesomeIcons.cloudRain, color: Color(0xFFB3E5FC), size: size,);
      }
    case "Snow":
      {
        return Icon(FontAwesomeIcons.snowman, color: color, size: size);
      }
    default: {return Icon(FontAwesomeIcons.sun, color: color, size: size,);}
  }
}









